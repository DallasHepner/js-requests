...
function reverseHelloButtonColor() {
...
sayHelloButton.addEventListener('mouseout', reverseHelloButtonColor)
...
const repeatMyParam = () => {
    // let repeatElem = document.getElementById('repeat-button');

    axios.get(`http://localhost:3000/repeat/Apex-Legends-ROCKS!`)
    .then(res => {
        document.getElementById('repeat-button').textContent = res.data
        // repeatElem.textContent = res.data;
        // repeatElem.style.display = 'block';
    })
}
...
// CODE HERE
const querySend = () => {
    axios.get(`http://localhost:3000/query-test/?myquery=apex-legnds-tutorial`)
    .then(res => {
        console.log(res.data);
    })
    .catch(err => {
...
document.getElementById('query-button').addEventListener('click', querySend)
